...
export default class ApiService {
     ...
     * PATCH request
     */
    public patch<T>(endpoint: string, body: any, config?: RequestConfig): Promise<T> {
        return this._request<T>(endpoint, {
            ...config,
            method: 'PATCH',
            body: JSON.stringify(body),
     ...
     */
    private async _request<T>(endpoint: string, config: RequestConfig): Promise<T> {
        const {
            params,
            timeout = 10000,
            skipAuth = false,
